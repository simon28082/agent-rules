...
# Inject or replace the <!--agent-rules:begin--> ... <!--agent-rules:end--> block
# in the target instructions.md file. Preserves all user content outside the block.
inject_instructions_block() {
  local instructions_md_path="$1"
  local content_file="$2"

  mkdir -p "$(dirname "$instructions_md_path")"

  if [[ ! -f "$instructions_md_path" ]]; then
    # No existing instructions.md; just copy our content
    cp "$content_file" "$instructions_md_path"
    return
  fi

  if grep -q '<!--agent-rules:begin-->' "$instructions_md_path"; then
...
install_opencode() {
  ...
  printf 'Installed instructions.md to %s\n' "$opencode_root/instructions.md"
}
...
install_antigravity() {
  ...
  # Inject instructions.md block
  inject_instructions_block "$agy_root/instructions.md" "$DIST/antigravity/instructions.md"

  printf 'Installed antigravity rules to %s\n' "$agy_rules"
  printf 'Installed antigravity skills to %s\n' "$agy_skills"
  printf 'Installed instructions.md to %s\n' "$agy_root/instructions.md"
}
