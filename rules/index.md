...
本仓库是 human-maintained personal agent rules kit。`rules/index.md` 只说明规则选择，不承载具体工程规范；实际回复与执行要求始终以 `rules/response.md` 为基础。
...
- `rules/versioning.md`：完成 feature/fix/release、修改可发布包契约、准备提交或写变更说明时加载。
...
- 用户当前明确指令优先。
- 仓库内更具体规则优先于本目录通用规则。
- 语言规则优先于 `rules/response.md` 中的泛化工程约束。
